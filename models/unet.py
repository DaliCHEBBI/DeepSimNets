...
class feature_extractor_with_UNET_dfc(LightningModule):
    def __init__(self,Inplanes,true1=1,false1=2,false2=8,NANS=0.0):
        super(feature_extractor_with_UNET_dfc, self).__init__()
        ...
        self.triplet_loss = nn.TripletMarginWithDistanceLoss(
            distance_function=lambda x, y:1.0 - F.cosine_similarity(x, y),reduction='none')
        ...
        self.model=UNet(init_features=self.inplanes)
    def training_step(self,batch,batch_idx):
        ...
        training_loss=self.triplet_loss(FeatsL,FeatsR_plus,FeatsR_minus)*MaskGlob.squeeze()
        #training_loss=_add_Intra_Class_Variance(FeatsL,FeatsR_plus,FeatsR_minus,MaskGlob,margin=self.triplet_loss.margin)
        training_loss=training_loss.sum().div(MaskGlob.count_nonzero()+1e-12)
        gc.collect()
    ...
    def validation_step(self,batch,batch_idx):
        ...
        # Mask Global = Mask des batiments + Mask des offsets bien definis 
        MaskGlob=Mask0*MaskOffPositive*MaskOffNegative
        validation_loss=self.triplet_loss(FeatsL,FeatsR_plus,FeatsR_minus)*MaskGlob.squeeze()
        #validation_loss=_add_Intra_Class_Variance(FeatsL,FeatsR_plus,FeatsR_minus,MaskGlob,margin=self.triplet_loss.margin)
        validation_loss=validation_loss.sum().div(MaskGlob.count_nonzero()+1e-12)
        gc.collect()
...
class UNETWithDecisionNetwork_LM(LightningModule):
    def __init__(self,Inplanes,true1=1,false1=2,false2=8,NANS=-999.0):
        super(UNETWithDecisionNetwork_LM, self).__init__()
        #self.device=torch.device("cuda:0")
        self.true1=true1
        self.false1=false1
        self.false2=false2
...
class UNETWithDecisionNetwork_LM_Weight(LightningModule):
    def __init__(self,Inplanes,true1=1,false1=2,false2=8,NANS=-999.0):
        super(UNETWithDecisionNetwork_LM_Weight, self).__init__()
        #self.device=torch.device("cuda:0")
        self.true1=true1
        self.false1=false1
        self.false2=false2
        self.nans=NANS
        self.criterion = nn.BCEWithLogitsLoss(reduction="none")
        self.criterion.pos_weight=torch.tensor([1.0]).to(device=self.device, dtype=torch.float)
        self.inplanes = Inplanes
        self.learning_rate=0.0005
        self.nbsteps=0
        self.save_hyperparameters()
    ...
    def training_step(self,batch,batch_idx):
        ...
        FeatsR_plus=torch.gather(FeatsR,-1,Offp)
        # Mask Global = Mask des batiments + Mask des offsets bien definis 
        weight_pos=Mask0*MaskOffPositive
        weight_neg= 1.0 - weight_pos
        weight=weight_neg.count_nonzero()/weight_pos.count_nonzero()
        self.criterion.pos_weight=torch.tensor([weight]).to(device=self.device, dtype=torch.float)
        #weight_pos=weight*weight_pos
        ref=self.decisionNet(torch.cat((FeatsL,FeatsR_plus),1))
        training_loss=self.criterion(ref, weight_pos.to(device=self.device, dtype=torch.float))
        if (torch.any(torch.isnan(training_loss))):
            raise Exception("nan values encountered in training loss ")
        #training_loss=_add_Intra_Class_Variance(FeatsL,FeatsR_plus,FeatsR_minus,MaskGlob,margin=self.triplet_loss.margin)
        training_loss=training_loss.mean()#.div(weight_pos.count_nonzero()+1e-20)
        gc.collect()
        self.log("training_loss",training_loss, on_epoch=True)
        return training_loss
    ...
    def validation_step(self,batch,batch_idx):
        ...
        #weight_pos=weight*weight_pos
        ref=self.decisionNet(torch.cat((FeatsL,FeatsR_plus),1))
        validation_loss=self.criterion(ref, weight_pos.to(device=self.device, dtype=torch.float))
        if (torch.any(torch.isnan(validation_loss))):
            raise Exception("nan values encountered in validation loss ")
        #training_loss=_add_Intra_Class_Variance(FeatsL,FeatsR_plus,FeatsR_minus,MaskGlob,margin=self.triplet_loss.margin)
        gc.collect()
        self.log("val_loss",validation_loss, on_epoch=True)
        if (weight_pos.count_nonzero().item()):
             Tplus=torch.masked_select(torch.sigmoid(ref),weight_pos.bool())
             Tmoins=torch.masked_select(torch.sigmoid(ref),weight_neg.bool())
             self.logger.experiment.add_histogram('distribution positive',Tplus,global_step=self.nbsteps)
             self.logger.experiment.add_histogram('distribution negative',Tmoins,global_step=self.nbsteps)
        self.nbsteps=self.nbsteps+1
        validation_loss=validation_loss.mean()#.div(all_weight.count_nonzero()+1e-20)
        return validation_loss
    def forward(self,x):
         f_all=self.feature(x)
         #print(f_all[0].shape)
