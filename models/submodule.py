...
class feature_extraction(nn.Module):
    ...
    def forward(self, x):
        ...
        print("ouput firstconv ",output.shape)
        output      = self.layer1(output)
        print("ouput layer1 ",output.shape)
        output_raw  = self.layer2(output)
        print("ouput raw  layer2 ",output_raw.shape)
        output      = self.layer3(output_raw)
        print("ouput   layer3 ",output.shape)
        output_skip = self.layer4(output)
        print("ouput skip   layer4 ",output_skip.shape)


        output_branch1 = self.branch1(output_skip)
        print("ouput branch avg pool 64 ",output_branch1.shape)
        output_branch1 = F.interpolate(output_branch1, (output_skip.size()[2],output_skip.size()[3]),mode='bilinear')
        print("ouput branch avg pool 64 after interpol ",output_branch1.shape)
        ...
        output_feature = F.interpolate(output_feature, (x.size()[2],x.size()[3]), mode='bilinear')
